...
<div class="device"><div class="device-top"><span class="dot" id="device-dot"></span>Desk connection</div><p id="device-status">Connecting to your board…</p><p><code id="device-ip">ESP32 · Local device</code></p></div>
...
<div class="bottom-grid"><div class="info-panel"><h3><svg aria-hidden="true"><use href="#i-chat"/></svg>A familiar team, a shorter path.</h3><div class="flow"><span class="node">Your desk</span><svg aria-hidden="true"><use href="#i-arrow"/></svg><span class="node">Agora</span><svg aria-hidden="true"><use href="#i-arrow"/></svg><span class="node">Your agent</span></div><p>Your agents keep running on your computer. Desk agent reaches them through your existing Agora bridges.</p></div><div class="info-panel"><h3><svg aria-hidden="true"><use href="#i-chip"/></svg>Made for the space beside your keyboard.</h3><p>After you send, the green LED blinks until the reply arrives. One beep means the message went out. A different beep means it came back. Hold the talk button to say it out loud instead: the LED stays lit while the mic is open, and the reply is read back through the speaker.</p></div></div>
...
<div class="chat-layout"><aside class="chat-rail" aria-label="Choose an agent"><div class="eyebrow">Your agents</div><div class="picker" id="picker"></div><div class="rail-note"><p>One conversation at a time. A little room to think.</p><button class="text-button" id="chat-settings">Agent settings<svg aria-hidden="true"><use href="#i-arrow"/></svg></button></div></aside>
...
function showSub(name) {
  ...
  $('#settings-intro').textContent={wifi:'Join the Wi-Fi network that can reach your Agora server.',agents:'Keep your bridges running on your computer. Connect each agent to its Agora channel here.',voice:'Hold the talk button and speak, or tap the mic in a conversation. Your voice becomes text, and replies can be read back through the desk speaker or this browser.'}[name];
...
const micBlockedWhy='Browsers only share the microphone with secure pages, and the board serves plain HTTP. In Chrome, add '+location.origin+' at chrome://flags/#unsafely-treat-insecure-origin-as-secure and reload, or switch chat audio to the desk hardware under Settings › Voice.';
...
function renderChat(){
  ...
  const a=agents.find(a=>a.id===current);$('#chat-name').textContent=a?.name||a?.title||'Your conversation';$('#chat-detail').textContent=a?.ready?'Connected through Agora':'Your agent, one connection away';$('#chat-state').textContent=waitingJob||sending?'Listening':a?.ready?'Configured':'Setup needed';$('#chat-state').classList.toggle('on',!!a?.ready);
  const log=$('#log');log.replaceChildren();const rows=logs(current);
  if(!rows.length){const empty=el('div','empty');empty.append(agentIcon(current),el('h2','',a?.ready?'A little thought. A fresh start.':'Let’s make a connection.'),el('p','',a?.ready?'Ask a question, share an idea, or pick up where your work left off.':agents.length?'Connect this agent to Agora to bring the conversation to your desk.':'Waiting for your board. Your agents will appear here once connected.'));
...
function renderAgents(){
  ...
  agents.forEach(agent=>{const form=el('form','panel');form.dataset.agentForm=agent.id;const head=el('div','panel-heading');head.append(el('h3','',(agent.name||agent.title)+' connection'),el('p','','Use the same details as the bridge running on your computer.'));
    const name=field('Display name',agent.name,{max:40,required:true,placeholder:agent.title});const agentId=field('Agent ID',agent.agent_id,{max:64,pattern:'[A-Za-z0-9_\\-]+',help:'Match the bridge’s AGENT_ID.',placeholder:'claude-cli'});const url=field('Agora URL',agent.url,{type:'url',required:true,placeholder:'http://192.168.1.20:4470',help:'The server address, without an /api path.'});const channel=field('Channel ID',agent.channel,{max:80,required:true,pattern:'[A-Za-z0-9_\\-]+',placeholder:'general-a1b2'});const token=field('Access token','',{type:'password',max:500,required:!agent.token_set,placeholder:agent.token_set?'Saved — leave blank to keep it':'Your Agora bearer token',help:'Saved tokens are never sent back to this page.'});token.input.autocomplete='new-password';const pair=el('div','field-pair');pair.append(name.wrap,agentId.wrap);const row=el('div','row');const status=el('p','note');status.setAttribute('role','status');const save=el('button','primary','Save connection');save.type='submit';row.append(status,save);form.append(head,pair,url.wrap,channel.wrap,token.wrap,row);
...
async function renderVoice(){
  ...
  let v;try{v=await api('/api/voice');}catch(e){root.replaceChildren(el('p','form-empty',e.message));return;}
  ...
  const creds=panel('Credentials','Provider keys used for speech. Values are write-only and stay on the board.');
  VOICE_CATALOG.providers.forEach(p=>{
    ...
    const line=el('div','key-input');const input=el('input');input.type='password';input.autocomplete='new-password';input.maxLength=500;input.placeholder=v.keys[p.id]?'Paste a new key to replace it':(p.id==='groq'?'gsk_…':'sk-…');
    const save=button('Save','primary sm');const test=button('Test','ghost sm');const clear=button('Clear','ghost sm');const status=el('p','note');status.setAttribute('role','status');
    ...
    test.addEventListener('click',async()=>{test.disabled=true;test.textContent='Testing…';note(status,'Checking the key with '+p.label+'…');try{await api('/api/voice/test',{provider:p.id},20000);note(status,p.label+' credentials work.');status.classList.add('good');}catch(err){note(status,err.message,true);}finally{test.disabled=false;test.textContent='Test';}});
  ...
  const stt=panel('Speech to text','Turns what you say into the message your agent gets.');
  ...
  const sttHint=el('p','note');stt.append(sttProvider.wrap,sttModel.wrap,sttHint);
  const tts=panel('Text to speech','Reads the reply back aloud. The talk button always speaks through the desk; in a conversation the speaker toggle decides.');
  ...
  const pair=el('div','field-pair');pair.append(accent.wrap,voice.wrap);tts.append(ttsProvider.wrap,pair,ttsModel.wrap,ttsHint);
  const talk=panel('Talk button','Hold the button on the desk and speak; let go to send.');
  const meta=el('div','voice-meta');[['Talk button','GPIO '+(v.button_pin??6),'Hold to talk, release to send.'],['Microphone',v.mic?'Ready':'Not detected','INMP441 on I2S.'],['Speaker',v.speaker?'Ready':'Not detected','MAX98357A on I2S.']].forEach(([k,val,sub])=>{const d=el('div');d.append(el('strong','',val),document.createTextNode(k+' · '+sub));meta.append(d);});
  const agentSel=selectField('Button talks to',(agents.length?agents:agentIds.map(id=>({id,name:id}))).map(a=>[a.id,(a.name||a.title||a.id)+(a.ready===false?' · setup needed':'')]),v.agent,'The chat page’s mic uses whichever agent is open there.');
  talk.append(meta,agentSel.wrap,featureNote);
  ...
  const syncTts=()=>{const p=ttsProvider.select.value;fill(ttsModel.select,VOICE_CATALOG.tts[p],v.tts_models[p]);const voices=ttsVoiceList();
    ...
    ttsHint.textContent=p==='groq'?(accent.select.value==='arabic'?'Arabic switches Groq to the Arabic Orpheus model and voices.':'On Groq, American and British share the English Orpheus voices. Replies are spoken in 200-character pieces.'):(/^tts-1/.test(ttsModel.select.value)?'Accent is applied on gpt-4o-mini-tts. tts-1 uses the voice’s built-in accent only.':'gpt-4o-mini-tts follows the accent you pick.');
  ...
  const page=panel('Chat page audio','Where the chat’s mic button listens and where spoken replies play. Saved in this browser only; the API keys stay on the board either way.');
  const modeSel=selectField('Microphone and speaker',[['browser','This browser'],['desk','The desk’s INMP441 and MAX98357A']],audioMode);const modeHint=el('p','note');
  const syncMode=()=>{const m=modeSel.select.value;modeHint.classList.toggle('bad',m==='browser'&&!micSupported);
    modeHint.textContent=m==='browser'?(micSupported?'Your browser records, the board forwards the clip for transcription and fetches the spoken reply as audio for this page to play.':micBlockedWhy+' Spoken replies still play here.'):(v.mic&&v.speaker?'The mic button and speaker toggle drive the desk hardware.':'The desk microphone or speaker was not detected; check the wiring before using this mode.');};
  ...
  syncStt();syncTts();stack.append(creds,stt,tts,talk,page);root.append(stack);
}
